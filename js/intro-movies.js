(() => {
  ...
  const scenes = {
    family: [
      ...
      ["I love my family! 💜","Я люблю свою семью!","I love my family!","grandmagrandpa.jpg"]
    ],
    home: [
  ...
  function styles(){
    ...
    s.textContent=`
      ...
      .movie-stage{position:relative;min-height:500px;border-radius:24px;overflow:hidden;background:#eaf4ff;display:flex;align-items:center;justify-content:center}.movie-main-image{width:100%;height:500px;display:block;object-fit:cover;object-position:center;transition:transform .55s ease,filter .35s ease,box-shadow .35s ease}.movie-main-image.listening{animation:imageListening 1.15s ease-in-out infinite;filter:saturate(1.14) brightness(1.06);box-shadow:inset 0 0 0 5px rgba(117,103,217,.7),inset 0 0 55px rgba(117,103,217,.32)}
      ...
      @media(max-width:700px){.intro-movie-grid{grid-template-columns:1fr 1fr}.movie-stage,.movie-main-image{min-height:520px;height:520px}.movie-bubble{font-size:18px;max-width:86%}.movie-en{font-size:23px}}@media(max-width:430px){.intro-movie-grid{grid-template-columns:1fr}.movie-controls{flex-wrap:wrap}.movie-stage,.movie-main-image{min-height:470px;height:470px}}
    `;
